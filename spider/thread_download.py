...
""" 
@author: https://cloud.tencent.com/developer/article/1731218
@file: thread_download.py
@version: 1.0
@time: 2021/07/24 17:08:46
...
class Download:
    def __init__(self, link, file_path, thread_num):
        ...
        self.threads = []
        self.file_size = 0
        self.fake_ua = fake_useragent.UserAgent()  # 设置 useragent

    def download(self):
        req = Request(url=self.link, method='GET')
        req.add_header('Accept', '*/*')
        req.add_header('User-Agent', self.fake_ua['random'])
