...
class RedisQueue:
    ...
    queue_name (str): 队列建议取名为 spider_name:spider
    """

    def __init__(self, client=None, db=0, queue_name='spider'):
        self.client = (StrictRedis(host='localhost', port=6379, db=db, password='redis@tx3')
    ...
    def push(self, element, force=False):
        ...
        elif not self.already_seen(element) or force:
            self.client.lpush(self.name, element)
            self.client.sadd(self.seen_set, element)
